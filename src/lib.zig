...
// BF16 shaders (emulated BF16 for all hardware)
const attention_bf16_spv = @embedFile("attention_bf16_spv");
const attention_bf16_native_spv = @embedFile("attention_bf16_native_spv");

// FP16 shaders (native FP16 for supported hardware)
const attention_fp16_native_spv = @embedFile("attention_fp16_native_spv");
...
export fn aule_init() callconv(.C) i32 {
    ...
    global_ctx = AttentionContext.initWithBackward(
        ...
        attention_bf16_spv, // BF16 emulated
        attention_bf16_native_spv, // BF16 native using SPV_KHR_bfloat16
        attention_fp16_native_spv, // FP16 native
    ...
    if (global_ctx) |*ctx| {
        if (ctx.vulkan_ctx) |engine| {
            ...
            return switch (shader_variant) {
                ...
                .bf16_native => if (engine.bf16_native_pipeline != null) @as(i32, 1) else 0,
                .fp16_native => if (engine.fp16_native_pipeline != null) @as(i32, 1) else 0,
            };
        }
    }
    return -1;
