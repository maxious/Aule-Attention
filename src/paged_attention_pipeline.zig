...
/// Extended push constants for paged attention shader
pub const PagedAttentionPushConstants = extern struct {
    batch_size: u32,
    num_heads: u32,
    seq_len: u32,
    head_dim: u32,
...
/// Requires 8 descriptor bindings: 0=Q, 1=Output, 4=RoPECos, 5=RoPESin, 6=BlockTable, 7=KVPool
pub const PagedAttentionPipeline = struct {
    ...
    const Self = @This();
    const BLOCK_SIZE: u32 = 32; // Must match shader BLOCK_SIZE
    ...
    pub fn init(ctx: *const VulkanContext, shader_code: []const u8) !Self {
        ...
        // 0=Q, 1=K(unused), 2=V(unused), 3=Output, 4=RoPECos, 5=RoPESin, 6=BlockTable, 7=KVPool
        const bindings = [_]vk.DescriptorSetLayoutBinding{
            ...
            .{ // Binding 1: K (unused in paged mode, but shader still references it)
                .binding = 1,
                .descriptor_type = .storage_buffer,
                .descriptor_count = 1,
                .stage_flags = .{ .compute_bit = true },
            ...
            .{ // Binding 2: V (unused in paged mode, but shader still references it)
                .binding = 2,
                .descriptor_type = .storage_buffer,
                .descriptor_count = 1,
                .stage_flags = .{ .compute_bit = true },
        ...
        // Pipeline layout with extended push constants
        const push_constant_range = vk.PushConstantRange{
            .stage_flags = .{ .compute_bit = true },
            .offset = 0,
            .size = @sizeOf(PagedAttentionPushConstants),
        };
        ...
        const pool_size = vk.DescriptorPoolSize{
            .type = .storage_buffer,
            .descriptor_count = 8, // 8 storage buffers
        };
        ...
        log.info("PagedAttentionPipeline initialized with 8 bindings", .{});
    ...
    /// Update all descriptor bindings for paged attention
    pub fn updateDescriptors(
        ...
        // Dummy buffer for unused K/V bindings (shader references them but doesn't use them)
        ...
        const writes = [_]vk.WriteDescriptorSet{
            ...
            .{
                ...
                .descriptor_type = .storage_buffer,
                .p_image_info = undefined,
                .p_buffer_info = @ptrCast(&buffer_infos[6]),
                .p_texel_buffer_view = undefined,
            },
            .{
                .dst_set = self.descriptor_set,
                ...
                .descriptor_type = .storage_buffer,
                .p_image_info = undefined,
                .p_buffer_info = @ptrCast(&buffer_infos[7]),
                .p_texel_buffer_view = undefined,
            },
        };
    ...
    /// Dispatch paged attention compute shader
    pub fn dispatch(
        self: *const Self,
        batch_size: u32,
        num_heads: u32,
        ...
        const push_constants = PagedAttentionPushConstants{
        ...
        // Workgroup dimensions (32x32 blocks for paged shader)
        ...
        self.ctx.vkd.cmdPushConstants(
            ...
            .{ .compute_bit = true },
            0,
            @sizeOf(PagedAttentionPushConstants),
